    ...
    <div className="space-y-6">
      ...
      {loading ? <p className="text-slate-400">Loading trainers…</p> : trainers.length === 0 ? <div className="rounded-3xl bg-slate-900 p-6 text-slate-400">No trainers are currently available.</div> :
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">{trainers.map((trainer) => {
          const capacity = Number(trainer.max_members || 20); const assigned = Number(trainer.assigned_members || 0); const acceptingAssignments = trainer.is_available === true || trainer.is_available === 1 || trainer.is_available === '1'; const active = trainer.is_active === true || trainer.is_active === 1 || trainer.is_active === '1'; const fullyBooked = assigned >= capacity; const available = active && acceptingAssignments && !fullyBooked; const status = fullyBooked ? 'Fully booked' : !acceptingAssignments ? 'Not accepting requests' : 'Available';
          return <article key={trainer.id} className="rounded-[2rem] border border-slate-800 bg-slate-900/95 p-6 shadow-xl shadow-slate-950/30">
            <div className="flex items-start justify-between gap-4"><div><h2 className="text-lg font-semibold text-white">{trainer.full_name}</h2><p className="mt-1 text-sm text-emerald-300">{trainer.specialization || 'General fitness'}</p></div><span className={`rounded-full px-3 py-1 text-xs ${available ? 'bg-emerald-500/15 text-emerald-300' : fullyBooked ? 'bg-amber-500/15 text-amber-300' : 'bg-slate-800 text-slate-400'}`}>{status}</span></div>
