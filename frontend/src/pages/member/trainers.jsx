...
function TrainersPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const [trainersRes, assignmentRes] = await Promise.allSettled([
          api.get('/trainers'),
          api.get('/trainer-assignments/my-assignment')
        ]);

        if (trainersRes.status === 'fulfilled') {
          setTrainers(trainersRes.value.data?.data || []);
        } else {
          setError('Unable to load trainers.');
        }

        if (assignmentRes.status === 'fulfilled') {
          setActiveAssignment(assignmentRes.value.data || null);
        }
      } finally {
        setLoading(false);
      }
  ...
  const getStatusDetails = (trainer) => {
    ...
    const active =
      ...
      trainer.is_active === '1';
    const fullyBooked = assigned >= capacity;
    const isCurrentTrainer = activeAssignment?.trainer_id === trainer.id;
    const available = active && acceptingAssignments && !fullyBooked && !activeAssignment;

    let statusLabel, statusColor, icon;
    if (isCurrentTrainer) {
      statusLabel = 'Your Trainer';
      statusColor = 'border-emerald-500/30 bg-emerald-500/20 text-emerald-300';
      icon = CheckCircleIcon;
    } else if (fullyBooked) {
      statusLabel = 'Fully booked';
      statusColor = 'border-amber-500/20 bg-amber-500/10 text-amber-300/70';
    ...
    return { available, statusLabel, statusColor, StatusIcon: icon, isCurrentTrainer };
  };
    ...
    >
      <div className="space-y-6">
        ...
        {loading ? (
          ...
          <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
            {trainers.map((trainer) => {
              const { available, statusLabel, statusColor, StatusIcon, isCurrentTrainer } =
                getStatusDetails(trainer);
              return (
                <article
                  key={trainer.id}
