...
const blank = { fullName: '', email: '', password: '', phone: '', specialization: '', experienceYears: '', maxMembers: '20', availabilityNote: '', personalTrainingCost: '' };

function TrainersAdminPage() {
  const [trainers, setTrainers] = useState([]); const [requests, setRequests] = useState([]); const [form, setForm] = useState(blank); const [error, setError] = useState(''); const [notice, setNotice] = useState(''); const [loading, setLoading] = useState(true);
  ...
  return <DashboardShell title="Trainer management" subtitle="Create trainers, review capacity, and approve member requests."><div className="space-y-6">
    ...
    <section className="rounded-[2rem] border border-slate-800 bg-slate-900/95 p-6"><h2 className="text-xl font-semibold text-white">Pending requests</h2>{loading ? <p className="mt-4 text-slate-400">Loading…</p> : requests.filter((r) => r.status === 'pending').length === 0 ? <p className="mt-4 text-sm text-slate-400">No pending trainer requests.</p> : <div className="mt-4 space-y-3">{requests.filter((r) => r.status === 'pending').map((r) => <div key={r.id} className="flex flex-col gap-3 rounded-2xl bg-slate-950 p-4 sm:flex-row sm:items-center sm:justify-between"><p className="text-sm text-slate-200"><span className="font-semibold">{r.member_name}</span> requested <span className="font-semibold">{r.trainer_name}</span></p><button onClick={() => approve(r.id)} className="rounded-2xl bg-emerald-600 px-4 py-2 text-sm font-semibold text-white">Approve</button></div>)}</div>}</section>
    <section className="rounded-[2rem] border border-slate-800 bg-slate-900/95 p-6"><h2 className="text-xl font-semibold text-white">Trainer workload</h2><div className="mt-4 overflow-x-auto"><table className="min-w-full text-left text-sm"><thead className="text-slate-500"><tr><th className="p-3">Trainer</th><th className="p-3">Specialization</th><th className="p-3">Availability</th><th className="p-3">Members</th></tr></thead><tbody className="divide-y divide-slate-800">{trainers.map((t) => <tr key={t.id}><td className="p-3 text-white">{t.full_name}</td><td className="p-3 text-slate-400">{t.specialization || 'General fitness'}</td><td className="p-3 text-slate-400">{t.is_available ? 'Available' : 'Unavailable'}</td><td className="p-3 text-slate-400">{t.assigned_members}/{t.max_members || 20}</td></tr>)}</tbody></table></div></section>
  </div></DashboardShell>;
}
export default function AdminTrainers() { return <ProtectedRoute allowedRoles={['admin']}><TrainersAdminPage /></ProtectedRoute>; }
