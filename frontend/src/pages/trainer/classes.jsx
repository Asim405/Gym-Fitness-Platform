...
function TrainerClassesPage() {
  ...
  return (
    <DashboardShell title="My classes" subtitle="Create, update and review your scheduled sessions.">
        ...
        <section className="rounded-[2rem] border border-slate-800 bg-slate-900/95 p-6 shadow-xl shadow-slate-950/40">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-400">Review upcoming classes that you manage.</p>
            </div>
            <span className="inline-flex rounded-full bg-slate-800 px-3 py-2 text-xs uppercase tracking-[0.25em] text-slate-400">
              {loading ? 'Loading…' : `${classes.length} found`}
            </span>
          </div>

          {loading ? (
            <div className="mt-8 text-slate-400">Loading class schedule…</div>
          ) : classes.length === 0 ? (
            <div className="mt-8 rounded-3xl bg-slate-950/80 p-6 text-slate-400">No classes found yet. Add classes from the backend or create a new session.</div>
          ) : (
                ...
                <tbody className="divide-y divide-slate-800">
                  {classes.map((cls) => (
                    <tr key={cls.id} className="odd:bg-slate-950 even:bg-slate-900">
                      ...
                      <td className="px-4 py-4 text-slate-400">{new Date(cls.end_time).toLocaleString()}</td>
