...
// Safe query wrapper for standard queries
async function dbQuery(sql, params = []) {
  const { sql: adaptedSql, params: adaptedParams } = adaptSql(sql, params);
  return pool.query(adaptedSql, adaptedParams);
}

// Safe query wrapper for transaction clients
async function clientQuery(client, sql, params = []) {
  const { sql: adaptedSql, params: adaptedParams } = adaptSql(sql, params);
  return client.query(adaptedSql, adaptedParams);
}
...
async function upsertProfile(req, res) {
  ...
  try {
    const user = await dbQuery(`SELECT id FROM users WHERE id = $1 AND role = 'trainer'`, [trainerId]);
    const userRows = user.rows || user[0] || [];
    if (!userRows.length) return res.status(404).json({ error: 'Trainer not found' });
    ...
    const profile = await dbQuery('SELECT * FROM trainer_profiles WHERE trainer_id = $1', [trainerId]);
    const profileRows = profile.rows || profile[0] || [];
...
async function requestTrainer(req, res) {
  ...
  try {
    ...
    const trainerRows = trainerResult.rows || trainerResult[0] || [];
    if (!trainerRows.length || !trainerRows[0].is_active || !trainerRows[0].is_available) {
      return res.status(400).json({ error: 'This trainer is not available for requests' });
    }

    const capacityResult = await dbQuery(
      `SELECT COUNT(*) AS assigned_count FROM trainer_assignments WHERE trainer_id=$1 AND status='active'`,
      [trainerId]
    );
    const capacityRows = capacityResult.rows || capacityResult[0] || [];
    if (Number(capacityRows[0].assigned_count) >= Number(trainerRows[0].max_members || 20)) {
      return res.status(409).json({ error: 'This trainer is currently at capacity' });
    }
    ...
    const activeAssignmentRows = activeAssignment.rows || activeAssignment[0] || [];
    if (activeAssignmentRows.length) return res.status(409).json({ error: 'You already have an active trainer assignment' });

    const existing = await dbQuery(
      `SELECT id FROM trainer_requests WHERE member_id=$1 AND trainer_id=$2 AND status='pending'`, [req.user.id, trainerId]
    );
    const existingRows = existing.rows || existing[0] || [];
    if (existingRows.length) return res.status(409).json({ error: 'A request for this trainer is already pending' });
    ...
    let request;
    if (pool.dbType === 'mysql') {
      const insertedId = created.insertId || created[0]?.insertId;
      const fetched = await dbQuery('SELECT * FROM trainer_requests WHERE id = $1', [insertedId]);
      request = (fetched.rows || fetched[0] || [])[0];
    } else {
      request = (created.rows || created[0] || [])[0];
    }
...
async function approveRequest(req, res) {
  ...
  try {
    const requestResult = await dbQuery('SELECT * FROM trainer_requests WHERE id = $1', [requestId]);
    const requestRows = requestResult.rows || requestResult[0] || [];
    if (!requestRows.length) {
      return res.status(404).json({ error: 'Trainer request not found' });
    }
    const item = requestRows[0];
    ...
    const activeRows = activeAssignment.rows || activeAssignment[0] || [];
    ...
    const client = await pool.connect();
    try {
      await clientQuery(client, 'START TRANSACTION');

      const insertSql = pool.dbType === 'mysql'
        ? 'INSERT INTO trainer_assignments (member_id, trainer_id, assigned_by, status) VALUES ($1, $2, $3, $4)'
        : 'INSERT INTO trainer_assignments (member_id, trainer_id, assigned_by, status) VALUES ($1, $2, $3, $4) RETURNING id';
      const insertParams = [item.member_id, item.trainer_id, req.user.id, 'active'];
      await clientQuery(client, insertSql, insertParams);

      const updateSql = 'UPDATE trainer_requests SET status = $1, reviewed_by = $2, reviewed_at = NOW(), updated_at = NOW() WHERE id = $3';
      const updateParams = ['approved', req.user.id, item.id];
      await clientQuery(client, updateSql, updateParams);

      await clientQuery(client, 'COMMIT');
    } catch (txErr) {
    ...
    const updated = await dbQuery('SELECT * FROM trainer_requests WHERE id = $1', [item.id]);
    const updatedRows = updated.rows || updated[0] || [];
...
async function createAssignment(req, res) {
  ...
  try {
    ...
    const member = await clientQuery(client, `SELECT id FROM users WHERE id=$1 AND role='member' AND is_active=TRUE`, [memberId]);
    const memberRows = member.rows || member[0] || [];
    if (!memberRows.length) {
      await clientQuery(client, 'ROLLBACK');
      return res.status(404).json({ error: 'Active member not found' });
    }

    const trainer = await clientQuery(
      client,
      `SELECT u.id, u.is_active, COALESCE(tp.is_available, TRUE) AS is_available,
              COALESCE(tp.max_members, 20) AS max_members
       FROM users u LEFT JOIN trainer_profiles tp ON tp.trainer_id=u.id
    ...
    const trainerRows = trainer.rows || trainer[0] || [];
    if (!trainerRows.length || !trainerRows[0].is_active || !trainerRows[0].is_available) {
      await clientQuery(client, 'ROLLBACK');
      return res.status(409).json({ error: 'Trainer is unavailable' });
    }

    const active = await clientQuery(client, `SELECT id FROM trainer_assignments WHERE member_id=$1 AND status='active'`, [memberId]);
    const activeRows = active.rows || active[0] || [];
    if (activeRows.length) {
      await clientQuery(client, 'ROLLBACK');
      return res.status(409).json({ error: 'Member already has an active trainer assignment' });
    }

    const workload = await clientQuery(client, `SELECT COUNT(*) AS assigned_count FROM trainer_assignments WHERE trainer_id=$1 AND status='active'`, [trainerId]);
    const workloadRows = workload.rows || workload[0] || [];
    if (Number(workloadRows[0].assigned_count) >= Number(trainerRows[0].max_members)) {
      await clientQuery(client, 'ROLLBACK');
      return res.status(409).json({ error: 'Trainer is at capacity' });
    }
    ...
    const insertSql = pool.dbType === 'mysql'
      ...
      : 'INSERT INTO trainer_assignments (member_id, trainer_id, assigned_by, status) VALUES ($1, $2, $3, $4) RETURNING id';
    const insertParams = [memberId, trainerId, req.user.id, 'active'];
    const inserted = await clientQuery(client, insertSql, insertParams);

    await clientQuery(client, 'COMMIT');

    let assignment;
    if (pool.dbType === 'mysql') {
      const insertedId = inserted.insertId || inserted[0]?.insertId;
      const fetched = await dbQuery('SELECT * FROM trainer_assignments WHERE id=$1', [insertedId]);
      assignment = (fetched.rows || fetched[0] || [])[0];
    } else {
      const newId = (inserted.rows || inserted[0] || [])[0]?.id;
      const fetched = await dbQuery('SELECT * FROM trainer_assignments WHERE id=$1', [newId]);
      assignment = (fetched.rows || fetched[0] || [])[0];
    }
    ...
    try { await clientQuery(client, 'ROLLBACK'); } catch (_) {}
    console.error(err);
    res.status(500).json({ error: 'Failed to create trainer assignment' });
  } finally {
    client.release();
...
async function updateRequestStatus(req, res) {
  const { status } = req.body;
  const found = await dbQuery('SELECT * FROM trainer_requests WHERE id=$1', [req.params.id]);
  const foundRows = found.rows || found[0] || [];
  if (!foundRows.length) return res.status(404).json({ error: 'Trainer request not found' });
  
  const item = foundRows[0];
  const allowed = (req.user.role === 'member' && item.member_id === req.user.id && status === 'cancelled') || 
                  req.user.role === 'admin' || 
                  (req.user.role === 'trainer' && item.trainer_id === req.user.id && status === 'rejected');
...
async function endAssignment(req, res) {
  const assignment = await dbQuery('SELECT * FROM trainer_assignments WHERE id=$1', [req.params.id]);
  const assignmentRows = assignment.rows || assignment[0] || [];
  if (!assignmentRows.length) return res.status(404).json({ error: 'Trainer assignment not found' });
  
  const item = assignmentRows[0];
  if (req.user.role === 'trainer' && item.trainer_id !== req.user.id) return res.status(403).json({ error: 'You can only manage your own assignments' });
  if (req.user.role === 'member' && item.member_id !== req.user.id) return res.status(403).json({ error: 'Access denied' });
