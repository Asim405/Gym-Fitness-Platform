...
async function ensureMySqlSchema() {
  const queries = MYSQL_SCHEMA.split(';').map((statement) => statement.trim()).filter(Boolean);
...
async function bootstrapDatabase() {
  ...
  const tableCheck = await pool.query('SHOW TABLES');
  const names = (tableCheck.rows || []).map((row) => Object.values(row)[0].toString().toLowerCase());
  const required = ['users', 'membership_plans', 'memberships', 'trainer_profiles', 'trainer_assignments', 'class_schedules', 'attendance', 'workout_plans', 'diet_plans', 'invoices', 'progress_metrics'];
...
if (require.main === module) {
  bootstrapDatabase()
    ...
    .catch((error) => {
      console.error('❌ MySQL bootstrap failed:', error.message);
      process.exit(1);
    });
}

module.exports = { ensureMySqlSchema, bootstrapDatabase, MYSQL_SCHEMA };
